...
import { Loader2, Briefcase, PlusCircle, Sparkles, Users, FileCheck2, ChevronDown, ChevronUp, Star, CalendarPlus, Search, MessageSquare, Trash2, UserSearch, ArrowRight, ArrowLeft, MoreVertical, ThumbsDown, Gift, User } from "lucide-react";
...
interface ScoredCandidate extends ScreenCandidateOutput {
  candidate: { id: string; name: string; profile: string; avatar?: string; };
}
...
const dashboardItems = [
    {
      ...
      description: "Create and publish a new job listing to attract top talent from our pool of candidates.",
      link: "?tab=postJob",
      linkText: "Post a Job"
    },
    {
      ...
      description: "View, manage, and start the AI screening process for all of your active job listings.",
      link: "?tab=postings", 
...
function RecruiterPortalContent() {
 ...
 const handlePostJob: SubmitHandler<JobPostingFormValues> = async (data) => {
  ...
  const handleScreeningForJob = async (job: Job) => {
      router.push('?tab=screeningResults');
      setIsScreening(true);
      setScreeningResults([]);
    ...
    <div className="container mx-auto py-8 md:py-12">
      <div className="text-center mb-12">
        <h1 className="font-headline text-4xl md:text-5xl font-bold">
          Welcome, <span className="text-primary">Recruiter!</span>
        </h1>
        <p className="text-lg text-muted-foreground mt-4 max-w-2xl mx-auto">
          This is your portal to find the best talent. Post jobs, screen candidates with AI, and manage your hiring pipeline.
        </p>
      </div>
        ...
        {dashboardItems.map((item) => (
          <Card key={item.title} className="group flex flex-col shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300">
            ...
            <div className="p-6 pt-0">
               <Button asChild className="w-full" onClick={() => router.push(item.link)}>
                <Link href={item.link}>
                  {item.linkText}
                                                        ...
                                                        <TableCell className="text-right">
                                                             <DropdownMenu>
                                                                ...
                                                                <DropdownMenuContent align="end">
                                                                    <DropdownMenuItem onClick={() => handleViewProfile(app.candidateId)}>
                        ...
                        <Card className="shadow-lg">
                            ...
                            {!isScreening && screeningResults.length === 0 && (
                                <div className="text-center text-sm text-muted-foreground h-48 flex flex-col items-center justify-center">
                                    ...
                                    <Button variant="link" onClick={() => router.push('?tab=postings')}>Go to My Postings to start screening</Button>
                                </div>
                            )}
